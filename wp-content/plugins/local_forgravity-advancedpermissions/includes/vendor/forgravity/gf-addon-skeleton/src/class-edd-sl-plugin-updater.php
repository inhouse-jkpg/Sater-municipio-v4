...
namespace CosmicGiant\Plugin_Skeleton;
...
class EDD_SL_Plugin_Updater {
	...
	public function show_update_notification( $file, $plugin ) {
		...
		/* translators: the plugin name. */
			esc_html__( 'There is a new version of %1$s available.', 'cosmicgiant' ),
			esc_html( $plugin['Name'] )
		);

		if ( ! current_user_can( 'update_plugins' ) ) {
			echo ' ';
			esc_html_e( 'Contact your network administrator to install the update.', 'cosmicgiant' );
		} elseif ( empty( $update_cache->response[ $this->name ]->package ) && ! empty( $changelog_link ) ) {
			echo ' ';
			printf(
			/* translators: 1. opening anchor tag, do not translate 2. the new plugin version 3. closing anchor tag, do not translate. */
				__( '%1$sView version %2$s details%3$s.', 'cosmicgiant' ),
			...
			echo ' ';
			printf(
				__( '%1$sView version %2$s details%3$s or %4$supdate now%5$s.', 'cosmicgiant' ),
				'<a target="_blank" class="thickbox open-plugin-details-modal" href="' . esc_url( $changelog_link ) . '">',
				esc_html( $update_cache->response[ $this->name ]->new_version ),
				'</a>',
			...
			printf(
				' %1$s%2$s%3$s',
				'<a target="_blank" class="update-link" href="' . esc_url( wp_nonce_url( $update_link, 'upgrade-plugin_' . $file ) ) . '">',
				esc_html__( 'Update now.', 'cosmicgiant' ),
				'</a>'
			);
		}
	...
	private function api_request( $_action, $_data ) {
		...
		if ( ! is_array( $edd_plugin_url_available ) || ! isset( $edd_plugin_url_available[ $store_hash ] ) ) {
			...
			if ( empty( $host ) ) {
				...
				$response = wp_remote_get( $test_url, array( 'timeout' => 15, 'sslverify' => $verify_ssl ) );
		...
		return $this->get_version_from_remote( $_action );
	}
	...
	public function show_changelog() {
		...
		if ( ! current_user_can( 'update_plugins' ) ) {
			wp_die( esc_html__( 'You do not have permission to install plugin updates', 'cosmicgiant' ), esc_html__( 'Error', 'cosmicgiant' ), array( 'response' => 403 ) );
		}
	 ...
	 *
	 * @params string $_action The requested action. The param was added by CosmicGiant.
	 *
	 * @return array|false
	 */
	private function get_version_from_remote( $action ) {
		global $wpdb;
		...
		if ( $action === 'plugin_latest_version' ) {
