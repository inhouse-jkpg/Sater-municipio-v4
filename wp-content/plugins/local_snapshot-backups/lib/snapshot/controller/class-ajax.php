...
class Ajax extends Controller {
	...
	public function save_snapshot_settings() {
		...
		if ( isset( $_POST['build_type'] ) && in_array( $_POST['build_type'], array( 'php_code', 'mysqldump' ), true ) ) {
			...
			if ( $_POST['build_type'] !== $old_method ) {
				$log->create(
					array(
						'action'  => 'snapshot_db_build_type_modified',
						'details' => 'Build type is set to: ' . sanitize_text_field( wp_unslash( $_POST['build_type'] ) ),
					)
				);
			}
			Settings::set_db_build_method( sanitize_text_field( wp_unslash( $_POST['build_type'] ) ) );
		}
		...
		if ( isset( $_POST['snapshot-backup-limit'] ) ) {
			...
			if ( isset( $_POST['existing_backup_limit'] ) && $limit !== $_POST['existing_backup_limit'] ) {
