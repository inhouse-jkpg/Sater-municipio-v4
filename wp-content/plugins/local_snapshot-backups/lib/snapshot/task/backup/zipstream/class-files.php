...
class Files extends Task\Backup\Zipstream {
	...
	public function zipstream_files( $model, $options ) {
		 ...
		 * @param bool $encoded Tell if these files are base64 encoded or not.
		 */
		...
		foreach ( $requested_files as $file ) {
			...
			// Decode back to normal file name if required.
			$file = $encoded ? $this->url_safe_base64_decode( $file ) : $file;
