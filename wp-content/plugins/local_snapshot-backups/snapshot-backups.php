...
if ( ! defined( 'SNAPSHOT_BACKUPS_VERSION' ) ) {
	define( 'SNAPSHOT_BACKUPS_VERSION', '4.21.1' );
}
...
$snapshotActivation = \WPMUDEV\Snapshot4\Activate::get_instance();

register_activation_hook(
	__FILE__,
	array( $snapshotActivation, 'boot' )
);
...
if ( ! function_exists( 'snapshot_onload' ) ) {
	...
	function snapshot_onload() {//phpcs:ignore Squiz.WhiteSpace.FunctionSpacing.BeforeFirst
		global $snapshotActivation;
		$plugin_version = get_option( 'snapshot_backups_version', '4.18.0' );

		if ( ! $plugin_version || version_compare( SNAPSHOT_BACKUPS_VERSION, $plugin_version, '>' ) ) {
			$snapshotActivation->invoke_maybe_create_snapshot_action_logs_table();
