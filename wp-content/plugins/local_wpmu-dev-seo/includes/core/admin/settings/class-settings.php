...
class Settings extends Admin_Settings {
	...
	public function validate( $input ) {
		...
		// Advanced Tools.
		$disable_adv_tools = 'disable-adv-tools';
		if ( isset( $input[ $disable_adv_tools ] ) ) {
			$autolink_options                       = self::get_component_options( self::COMP_AUTOLINKS );
			$autolink_options[ $disable_adv_tools ] = ! empty( $input[ $disable_adv_tools ] );
			self::update_component_options( self::COMP_AUTOLINKS, $autolink_options );
			unset( $autolink_options[ $disable_adv_tools ] );
		}

		if ( isset( $input['breadcrumb'] ) ) {
			$result['breadcrumb'] = $input['breadcrumb'];
		}

		if ( isset( $input['autolinks'] ) ) {
			$result['autolinks'] = $input['autolinks'];
		}
		...
		$booleans = array_keys( SC_Settings::get_known_components() );
	...
	public function activate_component() {
		...
		if ( isset( $data['wds-activate-component'] ) ) {
			...
			if ( 'adv-tools' === $component ) {
				$options                      = self::get_component_options( self::COMP_AUTOLINKS );
				$options['disable-adv-tools'] = false;

				self::update_component_options( self::COMP_AUTOLINKS, $options );
			} else {
	...
	private function get_plugin_modules() {
		...
		$disable_adv_tools = (bool) \smartcrawl_get_array_value(
			self::get_component_options( SC_Settings::COMP_AUTOLINKS ),
			'disable-adv-tools'
		);
		...
		$all_plugin_modules = array(
			...
			'autolinks' => $this->plugin_module_args(
				__( 'Advanced Tools', 'wds' ),
				'disable-adv-tools',
				true,
				$disable_adv_tools
			),
