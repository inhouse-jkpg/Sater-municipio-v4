...
class Assets extends Controller {
	...
	private function register_advanced_tools_scripts() {
		if ( ! \SmartCrawl\Redirects\Controller::get()->should_run() ) {
			return;
		}
		...
		wp_localize_script(
			...
			'_wds_redirects',
			array(
				'redirects'          => \SmartCrawl\Redirects\Controller::get()->get_redirects(),
				'nonce'              => wp_create_nonce( 'wds-redirects-nonce' ),
				'home_url'           => untrailingslashit( home_url( '/' ) ),
				'accepted-csv-types' => \smartcrawl_csv_mime_types(),
				'maxmind_license'    => GeoDB::get()->get_license(),
			)
		);
	...
	private function register_sitemap_page_scripts() {
		...
		wp_localize_script(
			...
			array(
				...
				'advanced_tools_url' => Admin_Settings::admin_url( Settings::TAB_AUTOLINKS ) . '&tab=tab_url_redirection',
			)
		);
	...
	private function register_dashboard_page_scripts() {
		...
		wp_localize_script(
			...
			array(
				'nonce'             => $nonce,
				'advanced_page_url' => Admin_Settings::admin_url( Settings::TAB_AUTOLINKS ),
			)
		);
	...
	private function register_metabox_scripts() {
		...
		$args['advanced']['redirect'] = array(
			'has_permission' => \user_can_see_seo_metabox_301_redirect() && \SmartCrawl\Redirects\Controller::get()->should_run(),
		);
		...
		if ( Settings::get_setting( 'autolinks' ) ) {
