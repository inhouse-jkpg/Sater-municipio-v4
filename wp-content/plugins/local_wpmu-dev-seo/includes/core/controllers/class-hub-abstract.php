...
use SmartCrawl\Admin\Settings\Advanced;
...
abstract class Hub_Abstract {
	...
	public function json_seo_summary() {
		...
		$moz            = array(
			'active' => $moz_active,
			'data'   => $moz_active
				? get_option( \SmartCrawl\SEOMoz\Cron::OPTION_ID, array() )
				: (object) array(),
		);

		// Robots file.
		$robots_controller = Robots::get();

		// The whole advanced page can be disabled in the network settings.
		$autolinks_settings = Advanced::get();
		$advanced_active    = Admin_Settings::is_tab_allowed( Settings::TAB_AUTOLINKS );
		...
		$advanced           = $advanced_active
			? array(
				...
				'autolinking_active' => Settings::get_setting( 'autolinks' ),
			)
			: array();
		...
		$schema_active         = ! \smartcrawl_get_array_value( $options, 'disable-schema' ) && Admin_Settings::is_tab_allowed( Settings::TAB_SCHEMA );
	...
	private function is_active( $module ) {
		return Settings::get_setting( $module ) && Admin_Settings::is_tab_allowed( 'wds_' . $module );
	}
