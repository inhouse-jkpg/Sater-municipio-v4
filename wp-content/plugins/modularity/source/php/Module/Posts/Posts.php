...
class Posts extends \Modularity\Module
{
    ...
    public function data(): array
    {
        ...
        $postsAndPaginationData       = null;
        if ($data['posts_data_source'] === 'input') {
            // Manual input: accordion HTML comes from the "data" repeater (mu-plugin prepareAccordion).
            // Do not build fake WP_Post rows — that duplicated work and could trigger preparePostObject.
            $data['posts']       = [];
            $data['stickyPosts'] = [];
        } else {
        ...
        if (
            $postsAndPaginationData !== null
            && !empty($this->fields['posts_pagination'])
            && $this->fields['posts_pagination'] === 'page_numbers'
        ) {
    ...
    public function template()
    {
        if (($this->fields['posts_data_source'] ?? '') === 'input') {
            $this->data['template']         = 'expandable-list';
            $this->data['posts_display_as'] = 'expandable-list';

            return apply_filters(
                'Modularity/Module/Posts/template',
                'expandable-list.blade.php',
                $this,
                $this->data,
                $this->fields
            );
    ...
    public function getInputData($data)
    {
        ...
        if (!empty($data) && is_array($data)) {
            foreach ($data as $post) {
                    // Create a new WP_Post object for each post array
                    global $wpdb;
                    $post_id = $post['ID'] ?? 0;

                    // Try to load an existing post if ID is set and valid
                    if ($post_id && $wp_post = get_post($post_id)) {
                        $content[] = $wp_post;
                    } else {
                        // Otherwise, create a "fake" WP_Post object from array data
                        $post_data = array(
                            'ID' => $post_id,
                        ...
                        $defaults = array(
                            ...
                            'post_modified' => $post_data['post_date'],
                            'post_parent' => 0,
                            'menu_order' => 0,
                            'post_mime_type' => '',
                            'comment_count' => 0,
