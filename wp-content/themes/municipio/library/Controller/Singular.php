...
class Singular extends \Municipio\Controller\BaseController
{
    ...
    public function init()
    {
        ...
        $this->data['eventData'] = $this->showEventData($this->data['post']->id);
    ...
    private function showPageTitleOnOnePage(int $postId)
    {
        ...
        private function showEventData(int $postId = 0) {
        $post_type = get_post_type($postId);
        if ($post_type !== 'events') {
            return false;
        }
        $event_data = array(
            'start_datum' => get_field('start_datum', $postId),
            'slut_datum'  => get_field('slut_datum', $postId),
            'plats'       => get_field('plats', $postId), 
            'pris'        => get_field('pris', $postId),
            'arrangor'    => get_field('arrangor', $postId)
        );

        return apply_filters(
            'Municipio/Controller/Singular/showEventData',
            $event_data,
            $postId
        );
    }
