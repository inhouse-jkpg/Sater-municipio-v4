...
<div class="o-grid{{ !empty($stretch) ? ' o-grid--stretch' : '' }}" {{!empty($freeTextFiltering) ? 'js-filter-container=' . $ID : ''}}>
    @card([
        'context' => $context ?? ['module.manual-input.accordion']
    ])
        @if (empty($hideTitle) && !empty($postTitle) && !$accordionSpacedSections)
        ...
        <div>
            ...
            @accordion([
                'spacedSections' => $accordionSpacedSections ?? false,
                'classList' => ['sater-miaf-accordion']
            ])
                @foreach ($manualInputs as $input)
                    @accordion__item([
                        'heading' => $input['accordionColumnValues'] ?? $input['accordion_column_values'] ?? $input['title'] ?? '',
